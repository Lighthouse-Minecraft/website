    ...
    <flux:callout color="blue" inline class="mb-6">
            ...
            <x-slot name="actions">
                <flux:modal.trigger name="view-blog-{{ $blog->id }}">
                    <flux:button>Read Full Blog</flux:button>
                </flux:modal.trigger>
            </x-slot>

            <flux:modal name="view-blog-{{ $blog->id }}" class="w-full md:w-3/4 xl:w-1/2">
                <flux:heading size="xl" class="mb-4">{{ $blog->title }}</flux:heading>
                ...
                @can('acknowledge', $blog)
                    <div class="w-full text-right mb-4">
                        <flux:button wire:click="acknowledgeBlog({{ $blog->id }})" size="sm" variant="primary">
                            Mark Blog As Read
                        </flux:button>
                    </div>
                @endcan
