...
7. **`payout-preview` state lost on Livewire poll.** The Finalizing step polls every 30 seconds. Each poll re-mounts all child components, resetting `$excludedUserIds` to `[]` in the payout-preview child. The parent's `$excludedPayoutUserIds` is also reset since the `payoutExcludedUsersChanged` event is only fired on toggle. This means exclusions made by a manager could silently reset if 30 seconds pass without interaction.
