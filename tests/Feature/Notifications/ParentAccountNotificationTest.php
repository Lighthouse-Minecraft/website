...
it('passes approval data for under-13 child', function () {
    ...
    expect($mail->subject)->toBe('Your Child Has Created a Lighthouse Account')
        ->and($mail->markdown)->toBe('mail.parent-account')
        ->and($mail->viewData['childName'])->toBe('YoungChild')
        ->and($mail->viewData['requiresApproval'])->toBeTrue();
});

it('passes non-approval data for 13+ child', function () {
    $child = User::factory()->minor(14)->create(['name' => 'TeenChild']);
    $notification = new ParentAccountNotification($child, requiresApproval: false);

    $mail = $notification->toMail(new AnonymousNotifiable);

    expect($mail->viewData['childName'])->toBe('TeenChild')
        ->and($mail->viewData['requiresApproval'])->toBeFalse();
});
